...
export default function Log(props: Props) {
  ...
  return (
    <div className="h-full w-full pb-10 pl-10 pr-8 pt-8 font-body">
      <ol className="h-full border-l-[2px] border-brunswick_green">
        {props.batchLog.log.map((log, idx) => (
          <li key={idx} className="pb-2" onClick={() => toggleExpansion(idx)}>
            <div className="flex items-start justify-between gap-4 align-top">
              <div className="z-10 -ml-[14px] flex-none">
                <LogBulletPoint
                  className="fill-brunswick_green "
                  bgFill={props.bgFill}
                />
              </div>
              <h4 className="-mt-2 flex-grow flex-col gap-1 text-xl leading-tight">
                {log.actorName}
                {props.showAddress && (
                  <p className="p-1 font-mono text-xs font-thin text-gray-400">
                    {log.actorAddress}
                  </p>
                )}
              </h4>
...
const LogBulletPoint = (
  props: SVGProps<SVGSVGElement> & { bgFill: string },
) => (
  <svg
  ...
  >
    <rect x={2} y={1} width={17} height={22} className={props.bgFill} />
